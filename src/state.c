	...
	switch(app->state)
	{
		...
		case LSS_STATE_GAME:
		{
			...
			if(app->game.done)
			{
				...
				app->leaderboard = t3net_get_leaderboard("https://www.tcubedsoftware.com/scripts/leaderboards/query.php", "live_studio_sessions", "0.1", buf[0], app->song_list->entry[app->selected_song]->id, 10, 0);
				if(!app->leaderboard)
				{
